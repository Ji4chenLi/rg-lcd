...
def log_validation(vae, unet, args, accelerator, weight_dtype, step):
    ...
    pipeline.load_lora_weights(lora_state_dict, adapter_name="default")
    pipeline.fuse_lora()
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.validation_steps == 0:
                        torch.cuda.empty_cache()
                        log_validation(
                            vae, unet, args, accelerator, weight_dtype, global_step
                        )
                        torch.cuda.empty_cache()

            logs = {
                "distill loss": distill_loss.detach().item(),
