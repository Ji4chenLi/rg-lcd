...
def get_wds_dataset(
    ...
    if work_on_latent:
        pipeline.extend(
            [
                wds.decode("l", handler=log_and_continue),
                wds.rename(
                    param="npy",
                    text="txt",
                ...
                wds.map_dict(
                    param=lambda param: param, text=lambda text: tokenizer(text)[0]
                ),
...
def get_data(args, preprocess_fns, epoch=0, tokenizer=None, work_on_latent=False):
    preprocess_train, preprocess_val = preprocess_fns
    data = {}

    if args.train_data or args.dataset_type == "synthetic":
        assert args.dataset_type == "webdataset"
