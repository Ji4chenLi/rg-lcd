...
class LatentCLIP(CLIP):
    ...
    def encode_image(self, image, normalize: bool = False):
        assert len(image.shape) == 4, f"Input must be a batch of images (NCHW), but got shape: {image.shape}"
        if image.shape[1] == 3:
            with torch.no_grad():
                latent = self.vae.encode(image).latent_dist.sample()
        elif image.shape[1] == 8:
            latent = DiagonalGaussianDistribution(image).sample()
        elif image.shape[1] == 4:
            latent = image

        features = self.visual(latent)
        return F.normalize(features, dim=-1) if normalize else features
