...
from .model import CLIP, CustomTextCLIP, LatentCLIP, convert_weights_to_lp, convert_to_custom_text_state_dict,\
...
from .transform import image_transform_v2, AugmentationCfg, PreprocessCfg, merge_preprocess_dict, merge_preprocess_kwargs, train_latent_clip_transform
...
def load_process_state_dict(model, checkpoint_path):
...
def load_checkpoint(model, checkpoint_path):
    state_dict = load_process_state_dict(model, checkpoint_path)
...
def create_model(
    ...
    train_latent_clip = "latent" in model_name
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        if pretrained:
            checkpoint_path = ''
            if train_latent_clip and not os.path.exists(pretrained):
                pretrained_cfg = get_pretrained_cfg("ViT-H-14", pretrained)
            else:
            ...
            if checkpoint_path:
                ...
                if train_latent_clip and not os.path.exists(pretrained):
                    ...
                    for k, v in state_dict.items():
                        if not ("visual" in k or "logit" in k):
                            new_state_dict[k] = v
...
def create_model_and_transforms(
    ...
    if "latent" in model_name:
        preprocess_train = train_latent_clip_transform(512)
